 ...
 */
class Version20170320150234 extends AbstractMigration
{
    /**
     * @param Schema $schema
     */
    public function up(Schema $schema)
    {
        ...
        $this->addSql(
            'CREATE OR REPLACE FUNCTION empenho.fn_saldo_dotacao_data_atual_empenho(character varying, integer, character varying, character varying, integer, character varying)
                 RETURNS numeric
                 LANGUAGE plpgsql
                AS $function$
                DECLARE
    ...
    {
        $this->abortIf($this->connection->getDatabasePlatform()->getName() != 'postgresql', 'Migration can only be executed safely on \'postgresql\'.');
        $this->addSql('DROP FUNCTION empenho.fn_saldo_dotacao_data_atual_empenho(character varying, integer, character varying, character varying, integer, character varying)');
    }
}
