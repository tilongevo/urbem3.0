 ...
 */
class Version20161129161134 extends AbstractMigration
{
    /**
     * @param Schema $schema
     */
    public function up(Schema $schema)
    {
        ...
        $this->addSql('ALTER TABLE public.sw_processo_apensado ALTER COLUMN "timestamp_apensamento" TYPE timestamp USING "timestamp_apensamento"::timestamp');
        $this->addSql('ALTER TABLE public.sw_processo_apensado ALTER COLUMN "timestamp_apensamento" SET NOT NULL');
        $this->addSql('ALTER TABLE public.sw_processo_apensado ALTER COLUMN "timestamp_apensamento" DROP DEFAULT');

        $this->addSql('CREATE VIEW sw_vw_consultaprocesso AS
   ...
   FROM ((((sw_processo p
    ...
    ((((sw_ultimo_andamento ultimo_andamento
     LEFT JOIN sw_recebimento ON ((((sw_recebimento.cod_andamento = ultimo_andamento.cod_andamento) AND (sw_recebimento.cod_processo = ultimo_andamento.cod_processo)) AND (sw_recebimento.ano_exercicio = ultimo_andamento.ano_exercicio))))
     LEFT JOIN sw_recibo_impresso ON ((((sw_recibo_impresso.cod_andamento = ultimo_andamento.cod_andamento) AND (sw_recibo_impresso.cod_processo = ultimo_andamento.cod_processo)) AND (sw_recibo_impresso.ano_exercicio = ultimo_andamento.ano_exercicio))))
     LEFT JOIN sw_assinatura_digital ON ((((sw_assinatura_digital.cod_andamento = ultimo_andamento.cod_andamento) AND (sw_assinatura_digital.cod_processo = ultimo_andamento.cod_processo)) AND (sw_assinatura_digital.ano_exercicio = ultimo_andamento.ano_exercicio))))
  ...
  WHERE (((((((((sw_processo_interessado.numcgm = sw_cgm.numcgm) AND (p.cod_usuario = usuario_inclusao.numcgm)) AND (p.cod_situacao = sw_situacao_processo.cod_situacao)) AND (p.cod_processo = ultimo_andamento.cod_processo)) AND (p.ano_exercicio = ultimo_andamento.ano_exercicio)) AND (p.cod_assunto = assunto.cod_assunto)) AND (p.cod_classificacao = assunto.cod_classificacao)) AND (ultimo_andamento.cod_usuario = usuario_ultimo_andamento.numcgm)) AND (p.cod_classificacao = classificacao.cod_classificacao));');
    ...
    public function down(Schema $schema)
    {
        ...
        $this->addSql('ALTER TABLE public.sw_processo_apensado ALTER COLUMN "timestamp_apensamento" TYPE date USING "timestamp_apensamento"::date');
        $this->addSql('ALTER TABLE public.sw_processo_apensado ALTER COLUMN "timestamp_apensamento" SET NOT NULL');
        $this->addSql('ALTER TABLE public.sw_processo_apensado ALTER COLUMN "timestamp_apensamento" DROP DEFAULT');
    }
}
