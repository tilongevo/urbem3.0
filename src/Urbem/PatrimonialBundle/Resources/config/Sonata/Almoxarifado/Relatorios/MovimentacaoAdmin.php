<?php

namespace Urbem\PatrimonialBundle\Resources\config\Sonata\Almoxarifado\Relatorios;

use Doctrine\Common\Collections\ArrayCollection;
use Doctrine\ORM\EntityRepository;
use Sonata\AdminBundle\Form\FormMapper;
use Sonata\AdminBundle\Route\RouteCollection;

use Symfony\Component\HttpFoundation\Request;
use Urbem\CoreBundle\Entity\Administracao\Mes;
use Urbem\CoreBundle\Entity\Almoxarifado\Almoxarifado;
use Urbem\CoreBundle\Entity\Almoxarifado\Catalogo;
use Urbem\CoreBundle\Entity\Almoxarifado\CatalogoItem;
use Urbem\CoreBundle\Entity\Almoxarifado\CentroCusto;
use Urbem\CoreBundle\Entity\Almoxarifado\Natureza;
use Urbem\CoreBundle\Entity\Almoxarifado\TipoItem;
use Urbem\CoreBundle\Entity\Compras\Fornecedor;
use Urbem\CoreBundle\Model\Patrimonial\Almoxarifado\AlmoxarifadoModel;
use Urbem\CoreBundle\Model\Patrimonial\Almoxarifado\CentroCustoModel;
use Urbem\CoreBundle\Resources\config\Sonata\AbstractSonataAdmin;

/**
 * Class MovimentacaoAdmin
 *
 * @package Urbem\PatrimonialBundle\Resources\config\Sonata\Almoxarifado\Relatorios
 */
class MovimentacaoAdmin extends AbstractSonataAdmin
{
    protected $baseRouteName = 'urbem_patrimonial_almoxarifado_relatorios';
    protected $baseRoutePattern = 'patrimonial/almoxarifado/relatorios';

    protected $legendButtonSave = [
        'icon' => 'receipt',
        'text' => 'Gerar Relatório',
    ];

    protected $includeJs = [
        '/patrimonial/javascripts/almoxarifado/relatorios/movimentacao/periodicidade.js',
        '/patrimonial/javascripts/almoxarifado/relatorios/movimentacao/tipo-relatorio.js',
        '/patrimonial/javascripts/almoxarifado/catalogo-classificacao-component.js',
        '/patrimonial/javascripts/almoxarifado/relatorios/item-estoque.js',
    ];

    /**
     * {@inheritdoc}
     */
    protected function configureRoutes(RouteCollection $collection)
    {
        $collection->clearExcept(['create']);

        $collection->add('movimentacao', 'movimentacao');
    }

    /**
     * {@inheritdoc}
     */
    protected function configureFormFields(FormMapper $formMapper)
    {
        $this->setLabel('label.almoxarifado.relatorios.relatorioItens');

        $this->setBreadCrumb();

        $entityManager = $this->getEntityManager();

        $centroCustoModel = new CentroCustoModel($entityManager);
        $almoxarifadoModel = new AlmoxarifadoModel($entityManager);

        $swCgm = $this->getCurrentUser()->getFkSwCgm();

        $fieldOptions = [];
        $fieldOptions['almoxarifados'] = [
            'attr'        => ['class' => 'select2-parameters '],
            'class'       => Almoxarifado::class,
            'data'        => new ArrayCollection($almoxarifadoModel->getAlmoxarifadosPadrao()),
            'label'       => 'label.almoxarifado.relatorios.almoxarifados',
            'mapped'      => false,
            'multiple'    => true,
            'placeholder' => 'label.selecione',
            'required'    => false,
        ];

        $fieldOptions['centrosCusto'] = [
            'attr'        => ['class' => 'select2-parameters '],
            'class'       => CentroCusto::class,
            'label'       => 'label.almoxarifado.relatorios.centrosCusto',
            'mapped'      => false,
            'multiple'    => true,
            'placeholder' => 'label.selecione',
            'required'    => false,
        ];

        $fieldOptions['catalogo'] = [
            'attr'         => ['class' => 'select2-parameters '],
            'class'        => Catalogo::class,
            'choice_value' => 'codCatalogo',
            'label'        => 'label.fornecedor.catalogo',
            'mapped'       => false,
            'placeholder'  => 'label.selecione',
            'required'     => false,
        ];

        $fieldOptions['centrosCusto'] = [
            'attr'          => ['class' => 'select2-parameters '],
            'class'         => CentroCusto::class,
            'label'         => 'label.almoxarifado.relatorios.centrosCusto',
            'mapped'        => false,
            'multiple'      => true,
            'placeholder'   => 'label.selecione',
            'query_builder' => $centroCustoModel->findCentroCustoPermissaoByCgmQuery($swCgm),
            'required'      => false,
        ];

        $fieldOptions['fornecedor'] = [
            'attr'                 => ['class' => 'select2-parameters '],
            'class'                => Fornecedor::class,
            'label'                => 'label.almoxarifado.relatorios.fornecedor',
            'json_from_admin_code' => $this->code,
            'json_query_builder'   =>
                function (EntityRepository $repo, $term, Request $request) {
                    $queryBuilder = $repo->createQueryBuilder('fornecedor');

                    $queryBuilder
                        ->join('fornecedor.fkSwCgm', 'swCgm')
                        ->join('fornecedor.fkComprasNotaFiscalFornecedores', 'notaFiscalFornecedor');

                    $queryBuilder
                        ->where(
                            $queryBuilder->expr()->like(
                                $queryBuilder->expr()->lower('swCgm.nomCgm'),
                                $queryBuilder->expr()->lower(':nomCgm')
                            )
                        )
                        ->setParameter('nomCgm', "%{$term}%");

                    return $queryBuilder;
                },
            'mapped'               => false,
            'multiple'             => false,
            'placeholder'          => $this->trans('label.selecione'),
            'required'             => false,
        ];

        $fieldOptions['numeroNotaFiscal'] = [
            'label'    => 'label.almoxarifado.relatorios.numeroNotaFiscal',
            'mapped'   => false,
            'required' => false,
        ];

        $fieldOptions['numeroSerie'] = [
            'label'    => 'label.almoxarifado.relatorios.numeroSerie',
            'mapped'   => false,
            'required' => false,
        ];

        $fieldOptions['item'] = [
            'attr'                 => ['class' => 'select2-parameters '],
            'class'                => CatalogoItem::class,
            'label'                => 'label.almoxarifado.relatorios.itemDe',
            'json_choice_label'    => function (CatalogoItem $catalogoItem) {
                return sprintf(
                    '%09d - %s - %s',
                    $catalogoItem->getCodItem(),
                    $catalogoItem->getFkAdministracaoUnidadeMedida()->getNomUnidade(),
                    $catalogoItem->getDescricao()
                );
            },
            'json_from_admin_code' => $this->code,
            'json_query_builder'   =>
                function (EntityRepository $repo, $term, Request $request) {
                    $queryBuilder = $repo->createQueryBuilder('catalogoItem');

                    $queryBuilder
                        ->join('catalogoItem.fkAdministracaoUnidadeMedida', 'unidadeMedida')
                        ->join('catalogoItem.fkAlmoxarifadoTipoItem', 'tipoItem')
                        ->join('catalogoItem.fkAlmoxarifadoCatalogoClassificacao', 'catalogoClassificacao')
                        ->join('catalogoClassificacao.fkAlmoxarifadoCatalogo', 'catalogo');

                    $queryBuilder
                        ->where(
                            $queryBuilder->expr()->like(
                                $queryBuilder->expr()->lower('catalogoItem.descricao'),
                                $queryBuilder->expr()->lower(':descricao')
                            )
                        )
                        ->setParameter('descricao', "%{$term}%");

                        if (intval($term) > 0) {
                            $queryBuilder->orWhere(
                                $queryBuilder->expr()->eq('catalogoItem.codItem', ':codItem')
                            )
                            ->setParameter('codItem', $term);
                        }
                        
                    return $queryBuilder;
                },
            'mapped'               => false,
            'required'             => false,

        ];

        $fieldOptions['itemDescricao'] = [
            'label'    => 'label.almoxarifado.relatorios.descricaoItem',
            'mapped'   => false,
            'required' => false,
        ];

        $fieldOptions['tipoItem'] = [
            'attr'         => ['class' => 'select2-parameters '],
            'class'        => TipoItem::class,
            'data'         => $this->modelManager->find(TipoItem::class, 0),
            'choice_label' => function (TipoItem $tipoItem) {
                if ($tipoItem->getCodTipo() == 0) {
                    return 'Todos';
                }

                return $tipoItem->getDescricao();
            },
            'choices'      => $this->getEntityManager()->getRepository(TipoItem::class)->findBy([], ['codTipo' => 'ASC']),
            'label'        => 'label.almoxarifado.relatorios.tipo',
            'mapped'       => false,
            'multiple'     => false,
            'required'     => true,
        ];

        $fieldOptions['ordenar'] = [
            'attr'     => ['class' => 'select2-parameters '],
            'choices'  => [
                'label.almoxarifado.relatorios.sort.classificacao' => 'catalogo_classificacao.cod_classificacao',
                'label.almoxarifado.relatorios.sort.item'          => 'catalogo_item.cod_item',
                'label.almoxarifado.relatorios.sort.descricaoItem' => 'catalogo_item.descricao',
            ],
            'data'     => 'classificacao',
            'label'    => 'label.almoxarifado.relatorios.sort.ordenarPor',
            'mapped'   => false,
            'required' => true,
        ];

        $fieldOptions['unidadeMedida'] = [
            'attr'       => ['class' => 'checkbox-sonata '],
            'choices'    => [
                'label.sigla'     => 1,
                'label.descricao' => 0,
            ],
            'data'       => 1,
            'label'      => 'label.almoxarifado.relatorios.unidadeMedida',
            'label_attr' => ['class' => 'checkbox-sonata '],
            'multiple'   => false,
            'expanded'   => true,
            'mapped'     => false,
            'required'   => true,
        ];

        $fieldOptions['demonstrativoNatureza'] = [
            'attr'       => ['class' => 'checkbox-sonata '],
            'choices'    => [
                'label.sigla'     => 1,
                'label.descricao' => 0,
            ],
            'data'       => 1,
            'label'      => 'label.almoxarifado.relatorios.demonstrativoNatureza',
            'label_attr' => ['class' => 'checkbox-sonata '],
            'multiple'   => false,
            'expanded'   => true,
            'mapped'     => false,
            'required'   => true,
        ];

        $fieldOptions['natureza'] = [
            'attr'        => ['class' => 'select2-parameters '],
            'class'       => Natureza::class,
            'label'       => 'label.almoxarifado.relatorios.natureza',
            'mapped'      => false,
            'multiple'    => true,
            'placeholder' => 'label.selecione',
            'required'    => false,
        ];

        $fieldOptions['tipoRelatorio'] = [
            'attr'       => ['class' => 'checkbox-sonata '],
            'choices'    => [
                'label.analitico' => 'analitico',
                'label.sintetico' => 'sintetico',
            ],
            'data'       => 'analitico',
            'label'      => 'label.tesouraria.relatorios.tipoRelatorio',
            'label_attr' => ['class' => 'checkbox-sonata '],
            'multiple'   => false,
            'expanded'   => true,
            'mapped'     => false,
            'required'   => true,
        ];

        $fieldOptions['quebraPor'] = [
            'attr'       => ['class' => 'checkbox-sonata '],
            'choices'    => [
                'label.almoxarifado.cgmAlmoxarifado' => 'almoxarifado',
                'label.centroCusto'                  => 'centrocusto',
                'label.marca'                        => 'marca',
            ],
            'label'      => 'label.almoxarifado.relatorios.quebrarPor',
            'label_attr' => ['class' => 'checkbox-sonata '],
            'multiple'   => true,
            'expanded'   => true,
            'mapped'     => false,
            'required'   => false,
        ];

        $fieldOptions['situacao'] = [
            'data'   => new \Datetime(),
            'format' => 'dd/MM/yyyy',
            'label'  => 'label.almoxarifado.relatorios.situacao',
            'mapped' => false,
        ];

        $fieldOptions['periodicidade'] = [
            'attr'     => ['class' => 'select2-parameters '],
            'choices'  => [
                'label.dia'       => 'dia',
                'label.mes'       => 'mes',
                'label.ano'       => 'ano',
                'label.intervalo' => 'intervalo',
            ],
            'data'     => 'intervalo',
            'label'    => 'label.almoxarifado.relatorios.periodicidade',
            'mapped'   => false,
            'required' => true,
        ];

        $fieldOptions['intervalo'] = [
            'label'               => false,
            'field_options_start' => [
                'format'   => 'dd/MM/yyyy',
                'label'    => 'De',
                'required' => true,
            ],
            'field_options_end'   => [
                'format'   => 'dd/MM/yyyy',
                'label'    => 'até',
                'required' => true,
            ],
            'mapped'              => false,
        ];

        $fieldOptions['mes'] = [
            'label'        => 'label.mes',
            'choice_label' => 'descricao',
            'class'        => Mes::class,
            'mapped'       => false,
            'placeholder'  => 'label.selecione',
        ];

        $fieldOptions['dia'] = [
            'label'  => 'label.dia',
            'format' => 'dd/MM/yyyy',
            'mapped' => false,
        ];

        $fieldOptions['ano'] = [
            'label'  => 'label.ano',
            'mapped' => false,
        ];

        $formMapper
            ->with('label.almoxarifado.relatorios.filtro')
            ->add('almoxarifados', 'entity', $fieldOptions['almoxarifados'])
            ->add('centrosCusto', 'entity', $fieldOptions['centrosCusto'])
            ->end();

        $formMapper
            ->with('label.fornecedor.catalogo')
            ->add('catalogo', 'entity', $fieldOptions['catalogo'])
            ->end();

        $formMapper
            ->with('label.item.tipoCadastroLoteClassificacao', [
                'class' => 'catalogoClassificacaoContainer',
            ])
            ->add('catalogoClassificacaoPlaceholder', 'text', [
                'mapped'   => false,
                'required' => false,
            ])
            ->end();

        $formMapper
            ->with('label.almoxarifado.relatorios.fornecedor')
            ->add('fornecedor', 'autocomplete', $fieldOptions['fornecedor'])
            ->add('numeroNotaFiscal', 'number', $fieldOptions['numeroNotaFiscal'])
            ->add('numeroSerie', 'text', $fieldOptions['numeroSerie'])
            ->end();

        $formMapper
            ->with('label.almoxarifado.relatorios.item')
            ->add('itemDe', 'autocomplete', $fieldOptions['item'])
            ->add('itemPara', 'autocomplete', array_merge($fieldOptions['item'], ['label' => 'label.almoxarifado.relatorios.itemPara']))
            ->add('itemDescricao', 'text', $fieldOptions['itemDescricao'])
            ->add('tipoItem', 'entity', $fieldOptions['tipoItem'])
            ->add('unidadeMedida', 'choice', $fieldOptions['unidadeMedida'])
            ->add('demonstrativoNatureza', 'choice', $fieldOptions['demonstrativoNatureza'])
            ->add('natureza', 'entity', $fieldOptions['natureza'])
            ->add('ordenar', 'choice', $fieldOptions['ordenar'])
            ->end();

        $formMapper
            ->with('label.almoxarifado.relatorios.periodicidade')
            ->add('periodicidade', 'choice', $fieldOptions['periodicidade'])
            ->add('intervalo', 'sonata_type_date_range_picker', $fieldOptions['intervalo'])
            ->add('dia', 'sonata_type_date_picker', $fieldOptions['dia'])
            ->add('mes', 'entity', $fieldOptions['mes'])
            ->add('ano', 'number', $fieldOptions['ano'])
            ->end();

        $formMapper
            ->with('label.tesouraria.relatorios.tipoRelatorio')
            ->add('tipoRelatorio', 'choice', $fieldOptions['tipoRelatorio'])
            ->add('quebraPor', 'choice', $fieldOptions['quebraPor'])
            ->add('situacao', 'sonata_type_date_picker', $fieldOptions['situacao'])
            ->end();

        $formMapper
            ->getFormBuilder()
            ->setAction('movimentacao');
    }
}
