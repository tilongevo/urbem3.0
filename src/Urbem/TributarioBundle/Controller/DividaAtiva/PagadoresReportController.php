...
/**
 * Class PagadoresReportController
 * @package Urbem\TributarioBundle\Controller\DividaAtiva
 */
class PagadoresReportController extends BaseController
{
    ...
    public function filtroAction(Request $request)
    {
        ...
        if ($form->isSubmitted() && $request->getMethod() == 'POST') {
            ...
            if ($tipo == strtolower(self::CREDITO)) {
                return $this->redirect($this->generateUrl('urbem_tributario_divida_ativa_relatorio_pagadores_credito'));
            } elseif ($tipo == strtolower(self::GRUPO_CREDITO)) {
                return $this->redirect($this->generateUrl('urbem_tributario_divida_ativa_relatorio_pagadores_grupo_credito'));
            }
        } else {
    ...
    public function generateForm()
    {
        ...
        $form = $this->createFormBuilder([])
            ->add(
                ...
                ChoiceType::class,
                [
                    'label' => 'label.dividaAtivaPagadoresReport.filtroPor',
            ...
            ->setAction($this->generateUrl('tributario_divida_ativa_relatorio_pagadores_filtro'))
            ->getForm();
