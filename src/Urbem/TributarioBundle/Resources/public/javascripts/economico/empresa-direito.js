	...
	var empresaDireito = {
		...
		initialize: function () {
			...
			if ((!uri[uri.length - 1].startsWith('create') && !uri[uri.length - 1].startsWith('edit'))
				&& !uri[uri.length - 2].startsWith('converter-empresa-direito')) {
				return;
			}

			atributoDinamicoParams = {
				entidade: 'CoreBundle:Economico\\CadastroEconomicoEmpresaDireito',
				fkEntidadeAtributoValor: 'getFkEconomicoAtributoEmpresaDireitoValores',
				codModulo: 14,
				codCadastro: 2
			};
