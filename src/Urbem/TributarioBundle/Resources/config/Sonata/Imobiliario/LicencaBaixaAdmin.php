...
class LicencaBaixaAdmin extends AbstractAdmin
{
    protected $baseRouteName = 'urbem_tributario_imobiliario_licencas_licenca_baixa';
    protected $baseRoutePattern = 'tributario/cadastro-imobiliario/licencas/licenca-baixa';
    ...
    protected function configureFormFields(FormMapper $formMapper)
    {
        ...
        $fieldOptions['dtInicio'] = [
            'label' => 'label.imobiliarioLicenca.dtBaixa',
            'mapped' => false,
            'format' => 'dd/MM/yyyy'
        ];
    ...
    public function prePersist($object)
    {
        ...
        $licencaBaixa = (new LicencaModel($em))->baixarLicenca($licenca, $this->getForm()->get('dtInicio')->getData(), $this->getForm()->get('motivo')->getData());

        if ($licencaBaixa instanceof LicencaBaixa) {
            $container->get('session')->getFlashBag()->add('success', $this->getTranslator()->trans('label.imobiliarioLicenca.msgBaixar', array('%licenca%' => (string) $licenca)));
