$(document).ready(function() {
    ...
    $('button[name="filter"]').click(function() {
        ...
        data.push({name: 'action', value: 'LoadContratoRescisaoAditivo'});

        modal.disableBackdrop().setTitle('Aguarde').setBody('Carregando').open();

        var url = UrlServiceProviderTCE + '&' + $(document.forms[0]).serialize();

        // src/Urbem/PrestacaoContasBundle/Resources/views/Tribunal/MG/Configuracao/ContratoRescisao/main.html.twig
        ...
        $listagem.DataTable({
            ...
            // src/Urbem/PrestacaoContasBundle/Service/Tribunal/MG/Configuracao/ConfiguracaoContratoRescisao.php::buildServiceProvider
            // src/Urbem/PrestacaoContasBundle/Service/Tribunal/MG/Configuracao/ConfiguracaoContratoRescisao.php::actionLoadContratoRescisao
            "ajax": {
                "url": url,
                "type": 'POST',
                "data": function (data) {
                    data.action = 'LoadContratoRescisao';
    ...
    $('a.new').click(function() {
        ...
        $.get(UrlServiceProviderTCE, data)
            .success(function (data) {
                modal.close().remove();
                modal.disableBackdrop().showCloseButton().setTitle('Dados para Configuração do Rescisão de Contrato').setBody($(data.form)).open({'size': 'large'});
            })
            .error(function (data) {
                modal.close().remove();
            });
    ...
    $('body').on('click', '.remove-row', function() {
        ...
        data = [];
        data.push({name: 'action', value: 'RemoveConfiguracaoContratoRescisao'});
        data.push({name: 'key', value: $(this).parent().find('input[name="key"]').val()});
    ...
    $('body').on('click', 'button[name="save_update"]', function(e) {
        ...
        $.post(UrlServiceProviderTCE, data)
            .success(function (data) {
                ...
                modal.disableBackdrop().showCloseButton().setTitle('Dados para Configuração da Rescisão de Contrato').setBody($(data.form)).open({'size': 'large'});
            })
            .error(function (data) {
                modal.close().remove();
            });
