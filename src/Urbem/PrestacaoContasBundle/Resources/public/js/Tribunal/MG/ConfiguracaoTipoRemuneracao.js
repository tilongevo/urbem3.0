$(document).ready(function() {
    ...
    $(document).on("change", "#" + UrbemSonata.uniqId + "_tipo_remuneracao_filter_entidade", function() {
    ...
    $(".save").on('click', function (e) {
        ...
        data.push({name: "action", value:"ValidateRemuneracao"});
    ...
    function load(entidade) {
        ...
        data = [];
        data.push({name: "action", value: "LoadTipoRemuneracao"});
        ...
        $.post(
            ...
            if (false === data.response) {
                $("#" + UrbemSonata.uniqId + "_tipo_remuneracao_filter_entidade").val('').change();
                modal.disableBackdrop()
                    .showCloseButton()
                    .setTitle("Erro")
                    .setBody(data.message)
            ...
            $.each(content, function (codTipo, items) {
                ...
                $("#" + UrbemSonata.uniqId + "_tipo_remuneracao_filter_registros_dynamic_collection_" + key + "_remuneracao").val(codTipo).change();
                $("#" + UrbemSonata.uniqId + "_tipo_remuneracao_filter_registros_dynamic_collection_" + key + "_evento").select2('val', items.cod_evento);
                key++;
            });
        });
    }
