$(document).ready(function() {
    ...
    $('button[name="filter"]').click(function() {
        ...
        data.push({name: 'action', value: 'LoadContratoAditivo'});
        ...
        $listagem.DataTable({
            ...
            "ajax": {
                ...
                "type": 'POST',
                "data": function (data) {
                    data.action = 'LoadContratoAditivo';
    ...
    $('a.new').click(function() {
        ...
        $.get(UrlServiceProviderTCE, data)
            .success(function (data) {
                modal.close().remove();
                modal.disableBackdrop().showCloseButton().setTitle('Dados para Configuração do Aditivo de Contrato').setBody($(data.form)).open({'size': 'large'});
            })
            .error(function (data) {
                modal.close().remove();
            });
    ...
    $('body').on('click', '.remove-row', function() {
        ...
        data = [];
        data.push({name: 'action', value: 'RemoveConfiguracaoContratoAditivo'});
        data.push({name: 'key', value: $(this).parent().find('input[name="key"]').val()});
    ...
    $('body').on('change', '#configuracao_contrato_aditivo_fkTcemgContratoAditivoTipo', function(e) {
        data = $("#form").serializeArray();
        data.push({name: 'action', value: 'LoadDynamicForm'});

        $.get(UrlServiceProviderTCE, data)
            .success(function (data) {
                $('.contrato_aditivo').html(data.unidade);
