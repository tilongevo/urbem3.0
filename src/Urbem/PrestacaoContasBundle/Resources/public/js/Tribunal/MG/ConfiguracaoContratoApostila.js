$(document).ready(function() {
    ...
    $('button[name="filter"]').click(function() {
        ...
        data.push({name: 'action', value: 'LoadContratoApostilaAditivo'});

        modal.disableBackdrop().setTitle('Aguarde').setBody('Carregando').open();

        var url = UrlServiceProviderTCE + '&' + $(document.forms[0]).serialize();

        // src/Urbem/PrestacaoContasBundle/Resources/views/Tribunal/MG/Configuracao/ContratoApostila/main.html.twig
        ...
        $listagem.DataTable({
            ...
            // src/Urbem/PrestacaoContasBundle/Service/Tribunal/MG/Configuracao/ConfiguracaoContratoApostila.php::buildServiceProvider
            // src/Urbem/PrestacaoContasBundle/Service/Tribunal/MG/Configuracao/ConfiguracaoContratoApostila.php::actionLoadContratoApostila
            "ajax": {
                "url": url,
                "type": 'POST',
                "data": function (data) {
                    data.action = 'LoadContratoApostila';
    ...
    $('a.new').click(function() {
        ...
        $.get(UrlServiceProviderTCE, data)
            .success(function (data) {
                modal.close().remove();
                modal.disableBackdrop().showCloseButton().setTitle('Dados para Configuração da Apostila de Contrato').setBody($(data.form)).open({'size': 'large'});
            })
            .error(function (data) {
                modal.close().remove();
            });
    ...
    $('body').on('click', '.edit-row', function() {
        ...
        $.get(UrlServiceProviderTCE, data)
            .success(function (data) {
                modal.close().remove();
                modal.disableBackdrop().showCloseButton().setTitle('Dados para Configuração do Apostila de Contrato').setBody($(data.form)).open({'size': 'large'});
            })
            .error(function (data) {
                modal.close().remove();
            });
    ...
    $('body').on('click', '.remove-row', function() {
        ...
        data = [];
        data.push({name: 'action', value: 'RemoveConfiguracaoContratoApostila'});
        data.push({name: 'key', value: $(this).parent().find('input[name="key"]').val()});
