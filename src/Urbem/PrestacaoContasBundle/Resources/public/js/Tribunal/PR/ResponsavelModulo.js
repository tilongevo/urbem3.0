$(document).ready(function() {
    ...
    $('button[name="filter"]').click(function() {
        ...
        data.push({name: 'action', value: 'LoadResponsavelModulo'});

        modal.disableBackdrop().setTitle('Aguarde').setBody('Carregando').open();

        var url = UrlServiceProviderTCE + '&' + $(document.forms[0]).serialize();

        // src/Urbem/PrestacaoContasBundle/Resources/views/Tribunal/PR/Configuracao/ResponsavelModulo/main.html.twig
        ...
        $listagem.DataTable({
            ...
            // src/Urbem/PrestacaoContasBundle/Service/Tribunal/PR/Configuracao/ConfiguracaoResponsavelModulo.php::buildServiceProvider
            // src/Urbem/PrestacaoContasBundle/Service/Tribunal/PR/Configuracao/ConfiguracaoResponsavelModulo.php::actionLoadResponsavelModulo
            "ajax": {
                "url": url,
                "type": 'POST',
                "data": function (data) {
                    data.action = 'LoadResponsavelModulo';
            ...
            // src/Urbem/PrestacaoContasBundle/Service/Tribunal/PR/Configuracao/ConfiguracaoResponsavelModulo.php::actionLoadResponsavelModulo
            "columns": [
                {"data": "fkTceprTipoModulo"},
                {"data": "fkTceprTipoResponsavelModulo"},
                {"data": "fkSwCgm"},
                {"data": "dataInicio"},
                {"data": "dataBaixa"},
                {"data": null}
    ...
    $('a.new').click(function() {
        ...
        $.get(UrlServiceProviderTCE, data)
            .success(function (data) {
                modal.close().remove();
                modal.disableBackdrop().showCloseButton().setTitle('Dados para Responsável do Módulo').setBody($(data.form)).open({'size': 'large'});
            })
            .error(function (data) {
                modal.close().remove();
            });
    ...
    $('body').on('click', '.remove-row', function() {
        ...
        data = [];
        data.push({name: 'action', value: 'RemoveConfiguracaoResponsavelModulo'});
        data.push({name: 'key', value: $(this).parent().find('input[name="key"]').val()});
