...
use Urbem\CoreBundle\Repository\Ldo\TipoIndicadoresRepository;
...
/**
 * Class LdoTipoIndicadoresType
 * @package Urbem\PrestacaoContasBundle\Form\Type
 */
class LdoTipoIndicadoresType extends AbstractType
{
    /**
     * @var SessionService
     */
    ...
    /**
     * LdoTipoIndicadoresType constructor.
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        $repo = $this->em->getRepository(TipoIndicadores::class);

        $builder->addModelTransformer(
            new CallbackTransformer(
                /* transform */
                function ($value) use ($repo, $options) {
                    $value = (new EntityTransform($repo, $this->em->getClassMetadata(TipoIndicadores::class)))->reverseTransform($value);
                ...
                function ($entidade) use ($repo, $options) {
                    $value = (new EntityTransform($repo, $this->em->getClassMetadata(TipoIndicadores::class)))->transform($entidade);
    ...
    {
        $resolver->setDefault('class', TipoIndicadores::class);
        $resolver->setDefault('multiple', false);
        $resolver->setDefault(
            'query_builder',
            function (TipoIndicadoresRepository $repository) {
