$(function () {
  ...
  var eventos = UrbemSonata.giveMeBackMyField('eventos'),
    ...
    situacao = UrbemSonata.giveMeBackMyField('stSituacao'),
    tipo = UrbemSonata.giveMeBackMyField('tipo'),
    stJuntarCalculo = UrbemSonata.giveMeBackMyField('stJuntarCalculo')
  ;

  if(situacao.val() == 'todos') {
    tipo.val('geral');
  }
  ...
  $('form').submit(function () {
      if ((situacao.val() == 'todos') && (tipo.val() != 'geral')) {
        mensagemErro(tipo, 'O tipo do filtro permitido para o cadastro Todos é somente Geral');
        return false;
      }
    return true;
  });
