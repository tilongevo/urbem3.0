...
class TransferenciaAnulacaoType extends LancamentoType
{
    public function execute(Lancamento $lancamento)
    {
        try {
            ...
            $sequencia = $suplementacaoModel->orcamentoAnulacaoSuplementacoesTransferencia(
                $lancamento->getExercicio(),
                $lancamento->getValor(),
                $lancamento->getDescricaoDecreto(),
                $codLote,
                'S',
                $lancamento->getEntidade(),
                SuplementacaoModel::CODIGO_HISTORICO_TRANSFERENCIA
            );
