...
class ExportarPagamentoBanrisulRepository
{
    ...
    */
    public function fetchGrupoDeContas()
    {
        $query = "
            SELECT
              CONCAT(configuracao_banrisul_conta.cod_convenio, '~', configuracao_banrisul_conta.cod_banco, '~', configuracao_banrisul_conta.cod_agencia, '~', configuracao_banrisul_conta.cod_conta_corrente),
              configuracao_banrisul_conta.*,
              conta_corrente.num_conta_corrente,
              agencia.num_agencia,
              agencia.nom_agencia,
              banco.num_banco,
              banco.nom_banco,
              CONCAT(configuracao_banrisul_conta.cod_convenio, '~', configuracao_banrisul_conta.cod_banco, '~', configuracao_banrisul_conta.cod_agencia, '~', configuracao_banrisul_conta.cod_conta_corrente) AS id
            FROM
              ima.configuracao_banrisul_conta
              INNER JOIN monetario.conta_corrente ON configuracao_banrisul_conta.cod_conta_corrente = conta_corrente.cod_conta_corrente
              AND configuracao_banrisul_conta.cod_agencia = conta_corrente.cod_agencia
              AND configuracao_banrisul_conta.cod_banco = conta_corrente.cod_banco
              INNER JOIN (
                SELECT
                  configuracao_banrisul_conta.cod_convenio,
                  max(
                ...
                FROM
                  ima.configuracao_banrisul_conta
                WHERE
                  configuracao_banrisul_conta.vigencia IN (
                    SELECT
                      ultima_vigencia_competencia.vigencia as dt_vigencia
                    FROM
                      (
                        SELECT
                          DISTINCT max(vigencia) as vigencia,
                          (
                            SELECT
                              cod_periodo_movimentacao
                            FROM
                    ...
                    ORDER BY
                      ultima_vigencia_competencia.vigencia DESC
                    LIMIT
                      1
                  )
                GROUP BY
                  configuracao_banrisul_conta.cod_convenio
              ) as max_configuracao_banrisul_conta ON configuracao_banrisul_conta.cod_convenio = max_configuracao_banrisul_conta.cod_convenio
              AND configuracao_banrisul_conta.timestamp = max_configuracao_banrisul_conta.timestamp
              INNER JOIN monetario.agencia ON conta_corrente.cod_agencia = agencia.cod_agencia
              AND conta_corrente.cod_banco = agencia.cod_banco
              INNER JOIN monetario.banco ON agencia.cod_banco = banco.cod_banco;";
    ...
    * @return array
    */
    public function fetchConfiguracaoOrgao($grupoContaId)
    {
        $query = "
            SELECT
              cod_convenio,
              ...
              cod_orgao
            FROM
              ima.configuracao_banrisul_orgao
            WHERE
              cod_convenio = :codConvenio
              AND cod_banco = :codBanco
              AND cod_agencia = :codAgencia
              AND cod_conta_corrente = :codContaCorrente
              AND timestamp = (
                SELECT
                  TO_CHAR(
                    MAX(timestamp), 'yyyy-mm-dd hh24:mi:ss.us'
        ...
        list($codConvenio, $codBanco, $codAgencia, $codContaCorrente) = explode('~', $grupoContaId);
        $sth->bindValue('codConvenio', $codConvenio, PDO::PARAM_INT);
        $sth->bindValue('codBanco', $codBanco, PDO::PARAM_INT);
        $sth->bindValue('codAgencia', $codAgencia, PDO::PARAM_INT);
        $sth->bindValue('codContaCorrente', $codContaCorrente, PDO::PARAM_INT);
        $sth->execute();
    ...
    * @return array
    */
    public function fetchCodConvenioBanco()
    {
        $query = "
            SELECT
              cod_convenio_banco
            FROM
              ima.configuracao_convenio_banrisul
            LIMIT 1;";
    ...
    public function fetchRemessaAtivos($filtro)
    {
        $query = "
            ...
            WHERE
              ...
              AND cod_orgao IN ({configuracaoOrgao})
            ORDER BY
              nom_cgm;";
        ...
        $configuracaoOrgaos = array_column($this->fetchConfiguracaoOrgao($filtro['grupoContasId']), 'cod_orgao');
        $query = str_replace('{configuracaoOrgao}', implode(',', $configuracaoOrgaos), $query);
    ...
    */
    public function fetchRemessaEstagiarios($filtro)
    {
        ...
        if ($filtro['tipoFiltro'] == ExportarPagamentoBanrisulAdmin::TIPO_FILTRO_ATRIBUTO_DINAMICO_ESTAGIARIO) {
        ...
        if ($filtro['tipoFiltro'] == ExportarPagamentoBanrisulAdmin::TIPO_FILTRO_CODIGO_ESTAGIO) {
        ...
        if ($filtro['tipoFiltro'] == ExportarPagamentoBanrisulAdmin::TIPO_FILTRO_LOTACAO) {
            $lotacao = sprintf('AND cod_orgao IN (%s)', implode(',', $filtro['valorFiltro']));
        }

        if ($filtro['tipoFiltro'] == ExportarPagamentoBanrisulAdmin::TIPO_FILTRO_LOCAL) {
            $codLocal = sprintf('AND cod_local in (%s)', implode(',', $filtro['valorFiltro']));
        }

        $configuracaoOrgaos = array_column($this->fetchConfiguracaoOrgao($filtro['grupoContasId']), 'cod_orgao');

        $query = strtr($query, ['{atributoEstagio}' => $atributoEstagio, '{codEstagio}' => $codEstagio, '{lotacao}' => $lotacao, '{codLocal}' => $codLocal, '{configuracaoOrgao}' => implode(',', $configuracaoOrgaos)]);
    ...
    */
    public function fetchRemessaPensaoJudicial($filtro)
    {
        ...
        if ($filtro['tipoFiltro'] == ExportarPagamentoBanrisulAdmin::TIPO_FILTRO_CGM_DEPENDENTE_PENSAO) {
            $cgms = !empty($filtro['cgms']) ? array_column($filtro['cgms'], 'numcgm') : [0];
            $numcgmDependente = sprintf('AND numcgm_dependente IN (%s)', implode(',', $cgms));
        }

        if ($filtro['tipoFiltro'] == ExportarPagamentoBanrisulAdmin::TIPO_FILTRO_CGM_MATRICULA_SERVIDOR) {
        ...
        $query = strtr($query, ['{desdobramento}' => $desdobramento, '{numcgmDependente}' => $numcgmDependente, '{codContratoServidor}' => $codContratoServidor, '{configuracaoOrgao}' => implode(',', $configuracaoOrgaos)]);
