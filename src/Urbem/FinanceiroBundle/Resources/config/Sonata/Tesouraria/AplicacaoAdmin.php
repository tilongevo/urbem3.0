...
class AplicacaoAdmin extends AbstractSonataAdmin
{
    protected $baseRouteName = 'urbem_financeiro_tesouraria_aplicacao';
    protected $baseRoutePattern = 'financeiro/tesouraria/aplicacao';
    ...
    public function createQuery($context = 'list')
    {
        ...
        $query->where(
            sprintf(
                ...
                Model\Tesouraria\TransferenciaModel::APLICACAO,
                $this->getExercicio()
            )
        );
    ...
    protected function configureDatagridFilters(DatagridMapper $datagridMapper)
    {
        $datagridMapper
            ->add(
                ...
                null,
                [
                    'label' => 'label.tesouraria.aplicacoes.codLote',
                ],
                'entity',
                [
                    ...
                    'query_builder' => function ($em) {
                        ...
                        $qb->where(sprintf('o.codTipo = %s', Model\Tesouraria\TransferenciaModel::APLICACAO));
    ...
    protected function configureListFields(ListMapper $listMapper)
    {
        ...
        $listMapper
            ...
            ->add(
                ...
                null,
                [
                    'label' => 'label.tesouraria.aplicacoes.codBoletim'
                ]
            )
            ->add(
                'nomEntidade',
                null,
                [
                    'label' => 'label.tesouraria.aplicacoes.nomEntidade'
                ]
            )
            ->add(
                'valor',
                'currency',
                [
                    'label' => 'label.tesouraria.aplicacoes.valor',
                    'currency' => 'BRL',
                    'attr' => [
                        'class' => "money"
                    ]
    ...
    protected function configureFormFields(FormMapper $formMapper)
    {
        ...
        $formMapper
            ...
            ->end()
            ->with('Dados para aplicação', ['class' => 'dados-arrecadacao'])
                ->add(
                    'codHistorico',
                    'autocomplete',
                    [
                        'label' => 'label.tesouraria.aplicacoes.codHistorico',
                        'multiple' => false,
                        'mapped' => false,
                ...
                ->add(
                    ...
                    'autocomplete',
                    [
                        'label' => 'label.tesouraria.aplicacoes.contaDebito',
                        'multiple' => false,
                        'mapped' => false,
                        'route' => ['name' => 'tesouraria_outras_operacoes_get_contas_by_entidade'],
                        'req_params' => [
                            'codEntidade' => 'varJsCodEntidade',
                            'tipoOperacao' => Model\Tesouraria\TransferenciaModel::APLICACAO,
                        ]
                    ]
                )
                ->add(
                    ...
                    'autocomplete',
                    [
                        'label' => 'label.tesouraria.aplicacoes.contaCredito',
                        'multiple' => false,
                        'mapped' => false,
                        'route' => ['name' => 'tesouraria_outras_operacoes_get_contas_by_entidade'],
                        'req_params' => [
                ...
                ->add(
                    ...
                    'textarea',
                    [
                        'label' => 'label.tesouraria.aplicacoes.observacao',
                        'mapped' => false,
                        'required' => false,
                    ]
                )
    ...
    public function saveRelationships($transferencia)
    {
        ...
        $transferencia = $transferenciaModel->parseTransferencia(
            ...
            $formContent,
            $exercicio,
            $tipoMovimento = Model\Tesouraria\TransferenciaModel::APLICACAO
        );

        // Processa Lancamento
        $transferenciaRepository->gerarLancamento($transferencia);
    ...
    public function prePersist($object)
    {
        ...
        try {
            ...
            $container->get('session')->getFlashBag()->add('success', "Aplicação realizada");
            $this->forceRedirect(
                "/financeiro/tesouraria/aplicacao/list"
            );
        } catch (\Exception $e) {
            $container->get('session')->getFlashBag()->add('error', Exception\Error::ERROR_PERSIST_DATA);
            $this->forceRedirect(
                "/financeiro/tesouraria/aplicacao/create"
            );
        }
    }
}
